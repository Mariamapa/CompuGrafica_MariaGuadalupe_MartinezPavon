//Animación por KeyFrames en OpenGL                                     María Guadalupe Martínez Pavón
...
typedef struct _frame {
	// Rotación del perro completo sobre Y
	float rotDog;
	float rotDogInc;

	// Posición del perro en el espacio
	float dogPosX;
	float dogPosY;
	float dogPosZ;
	float incX;
...
void DoMovement() {
	...
	// --- Rotación del cuerpo completo sobre Y (giro lateral) ---
	if (keys[GLFW_KEY_2]) {
		rotDog += 1.0f;
	}
	if (keys[GLFW_KEY_3]) {
...
void Animation() {
	...
	if (play)
	{
		...
		{
			//Draw animation
			// Dentro del else de interpolación activa:
